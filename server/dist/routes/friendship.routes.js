...
friendshipRouter.post("/requests", requireAuth, csrfGuard, validate({ body: z.object({ receiverId: z.string(), message: z.string().optional() }) }), asyncHandler(async (req, res) => {
...
friendshipRouter.post("/requests/:requestId/respond", requireAuth, csrfGuard, validate({ body: z.object({ action: z.enum(["ACCEPTED", "REJECTED"]) }) }), asyncHandler(async (req, res) => {
    const record = await respondToFriendRequest(String(req.params.requestId), req.user.id, req.body.action);
...
friendshipRouter.post("/requests/:requestId/cancel", requireAuth, csrfGuard, asyncHandler(async (req, res) => {
    const record = await cancelFriendRequest(String(req.params.requestId), req.user.id);
    res.json({ success: true, request: record });
}));
friendshipRouter.delete("/:targetUserId", requireAuth, csrfGuard, asyncHandler(async (req, res) => {
    await removeFriend(req.user.id, String(req.params.targetUserId));
    res.json({ success: true });
}));
friendshipRouter.post("/:targetUserId/block", requireAuth, csrfGuard, asyncHandler(async (req, res) => {
    const record = await blockUser(req.user.id, String(req.params.targetUserId));
...
friendshipRouter.post("/:targetUserId/unblock", requireAuth, csrfGuard, asyncHandler(async (req, res) => {
    const record = await unblockUser(req.user.id, String(req.params.targetUserId));
...
friendshipRouter.post("/:targetUserId/mute", requireAuth, csrfGuard, validate({ body: z.object({ mutedUntil: z.string().datetime().nullable().optional() }) }), asyncHandler(async (req, res) => {
    ...
    const record = await muteFriend(req.user.id, String(req.params.targetUserId), mutedUntil);
...
friendshipRouter.post("/:targetUserId/favorite", requireAuth, csrfGuard, validate({ body: z.object({ favorite: z.boolean().default(true) }) }), asyncHandler(async (req, res) => {
    const record = await favoriteFriend(req.user.id, String(req.params.targetUserId), req.body.favorite);
    res.json({ success: true, friendship: record });
}));
