...
messageRouter.post("/direct", requireAuth, csrfGuard, validate({ body: z.object({ targetUserId: z.string() }) }), asyncHandler(async (req, res) => {
    const conversation = await getOrCreateDirectConversation(req.user.id, req.body.targetUserId, req.user.isAdmin);
    res.json({ success: true, conversation });
}));
messageRouter.post("/send", requireAuth, csrfGuard, validate({ body: z.object({ targetUserId: z.string(), content: z.string().min(1), type: z.enum(["TEXT", "IMAGE", "VIDEO", "FILE", "VOICE_NOTE", "SYSTEM"]).optional(), replyToId: z.string().optional() }) }), asyncHandler(async (req, res) => {
