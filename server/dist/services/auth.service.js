...
const buildPayload = (user, sessionId) => ({
    sub: user.id,
...
export const registerUser = async (input) => {
    const existingUser = await prisma.user.findFirst({
        where: {
            OR: [{ username: input.username }, input.email ? { email: input.email } : undefined, input.phoneNumber ? { phoneNumber: input.phoneNumber } : undefined].filter(Boolean)
        }
    });
    if (existingUser) {
...
export const refreshUserSession = async (refreshToken) => {
    ...
    if (!tokenMatches)
        throw Object.assign(new Error("Session rotated"), { statusCode: 401 });
