...
export const respondToFriendRequest = async (requestId, receiverId, action) => {
...
export const muteFriend = async (userId, friendId, mutedUntil) => {
    const pair = normalizePair(userId, friendId);
    return prisma.friendship.updateMany({ where: pair, data: { mutedUntil } });
};
export const favoriteFriend = async (userId, friendId, favorite = true) => {
    const pair = normalizePair(userId, friendId);
...
export const canUsersMessage = async (actorId, targetUserId, actorIsAdmin = false) => {
    if (actorIsAdmin)
        return true;
    const blocked = await prisma.blockedUser.findFirst({
        where: {
