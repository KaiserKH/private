...
const normalizePair = (userA, userB) => (userA < userB ? [userA, userB] : [userB, userA]);
export const getOrCreateDirectConversation = async (userId, targetUserId, actorIsAdmin = false) => {
...
export const listMessages = async (conversationId, cursor) => {
...
export const editMessage = async (messageId, userId, content, actorIsAdmin = false) => {
    const message = await prisma.message.findUnique({ where: { id: messageId } });
    if (!message)
        throw Object.assign(new Error("Message not found"), { statusCode: 404 });
    if (message.senderId !== userId && !actorIsAdmin)
        throw Object.assign(new Error("Forbidden"), { statusCode: 403 });
    return prisma.message.update({ where: { id: messageId }, data: { content, isEdited: true } });
};
export const deleteMessageForEveryone = async (messageId, userId, actorIsAdmin = false) => {
    const message = await prisma.message.findUnique({ where: { id: messageId } });
    if (!message)
        throw Object.assign(new Error("Message not found"), { statusCode: 404 });
    if (message.senderId !== userId && !actorIsAdmin)
